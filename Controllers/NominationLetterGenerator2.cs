...
public class NominationLetterGenerator2
{
    public string GenerateNominationLetter(Employee employee)
    {
        // Define the path to the template and the output file
        string templatePath = HttpContext.Current.Server.MapPath("~/Documents/CEPGAFF.docx");
        string outputDirectory = HttpContext.Current.Server.MapPath("~/GeneratedLetters/");
        string outputPath = Path.Combine(outputDirectory, "NominationLetterCEPGAFF_" + employee.FullName + ".docx");
