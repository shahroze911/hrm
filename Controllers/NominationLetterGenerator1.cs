...
public class NominationLetterGenerator1
{
    public string GenerateNominationLetter(Employee employee)
    {
        // Define the path to the template and the output file
        string templatePath = HttpContext.Current.Server.MapPath("~/Documents/EPF_Form_2.docx");
        string outputDirectory = HttpContext.Current.Server.MapPath("~/GeneratedLetters/");
        string outputPath = Path.Combine(outputDirectory, "AppointmentLetterEPF_" + employee.FullName + ".docx");
